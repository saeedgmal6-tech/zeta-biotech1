...
function ready(){
  ...
  if(panel.dataset.zetaCloseFixed!=='1'){
    ...
    if(close)close.addEventListener('click',function(e){
      e.preventDefault();e.stopPropagation();panel.classList.remove('show');panel.setAttribute('aria-hidden','true');button.setAttribute('aria-expanded','false');
    });
  }
  ...
  root.querySelectorAll('.zeta-ai-product').forEach(function(a){
    ...
    a.addEventListener('click',function(e){
      e.preventDefault();e.stopPropagation();var el=document.getElementById('products');
      ...
      panel.classList.remove('show');button.setAttribute('aria-expanded','false');
    });
  });
  ...
  if(button.dataset.zetaDragFixed!=='1'){
    ...
    try{var saved=JSON.parse(localStorage.getItem('zeta-ai-position')||'null');if(saved&&Number.isFinite(saved.left)&&Number.isFinite(saved.top)){root.style.left=Math.max(0,Math.min(window.innerWidth-root.offsetWidth,saved.left))+'px';root.style.top=Math.max(0,Math.min(window.innerHeight-root.offsetHeight,saved.top))+'px';root.style.right='auto';root.style.bottom='auto'}}catch(x){}
  }
  return true;
}
