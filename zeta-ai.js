...
var aliases={
 ...
 'منتج':['product','products','منتج','منتجات','دواء','ادوية'], 'منتجات':['product','products','منتج','منتجات'], 'دواء':['product','products','medicine','drug'], 'ادوية':['product','products','medicine'],
...
function score(item,ts,q){var t=norm(item.title+' '+item.raw),title=norm(item.title),n=0;ts.forEach(function(z){if(t.indexOf(z)>=0)n+=3;if(title.indexOf(z)>=0)n+=8;(aliases[z]||[]).forEach(function(s){if(t.indexOf(norm(s))>=0)n+=5;});});var nq=norm(q);if(nq.length>3&&t.indexOf(nq)>=0)n+=25;return n;}
...
function answer(q){var ts=terms(q);if(!ts.length)return ar()?'اكتب سؤالك، وسأبحث في كل معلومات الموقع.':'Ask your question and I will search the website knowledge.';var ranked=knowledge.map(function(x){return {item:x,score:score(x,ts,q)}}).filter(function(x){return x.score>0}).sort(function(a,b){return b.score-a.score});if(!ranked.length)return fallback(q);var best=ranked[0].item,s=best.source||{},out='';if(best.type==='faq')out=pick(s.answerAr,s.answerEn)||textOf(s);else if(best.type==='product')out=productText(s);else if(best.type==='company')out=pick(s.overviewAr,s.overviewEn)||pick(s.overview?.ar,s.overview?.en)||pick(s.descriptionAr,s.descriptionEn)||textOf(s);else if(best.type==='job')out=pick(s.descriptionAr,s.descriptionEn)||pick(s.titleAr,s.titleEn)||textOf(s);else if(best.type==='website')out=ar()?'وجدت هذه المعلومات في المحتوى المنشور على الموقع:<br><br>'+esc(best.raw.slice(0,1800)):'I found this information in the published website content:<br><br>'+esc(best.raw.slice(0,1800));else out=pick(s.descriptionAr,s.descriptionEn)||pick(s.bodyAr,s.bodyEn)||pick(s.textAr,s.textEn)||textOf(s);var related=ranked.slice(1,4).map(function(r){return '• '+label(r.item.type)+': '+esc(r.item.title)}).join('<br>');if(related)out+='<br><br><b>'+(ar()?'معلومات مرتبطة:':'Related information:')+'</b><br>'+related;return out||fallback(q)}
function renderAssistant(){
 var old=document.getElementById('znextAssistant');
 if(old && old.getAttribute('data-zeta-ai')!=='true') old.remove();
 var host=document.getElementById('znextAssistant');
 if(!host){host=document.createElement('div');host.id='znextAssistant';host.className='znext-assistant';document.body.appendChild(host)}
 host.setAttribute('data-zeta-ai','true');
 host.innerHTML='<div class="znext-ai-panel" id="znextAiPanel"><div class="znext-ai-head">🤖 ZETA BIOTECH AI</div><div class="znext-ai-body" id="znextAiBody"><div class="znext-ai-msg">'+(ar()?'مرحبًا! أنا مساعد ZETA BIOTECH. أبحث في معلومات الشركة والمنتجات والبروشورات والأخبار والشهادات والوظائف وFAQ ومحتوى الموقع الظاهر.':'Hello! I search ZETA BIOTECH company, products, brochures, news, certifications, careers, FAQs and visible website content.')+'</div></div><form class="znext-ai-form" id="znextAiForm"><input id="znextAiInput" autocomplete="off" placeholder="'+(ar()?'اسأل عن أي معلومة في الموقع...':'Ask about anything on the website...')+'"><button>↗</button></form></div><button class="znext-ai-btn" id="znextAiBtn">🤖 '+(ar()?'المساعد الذكي':'AI Assistant')+'</button></div>';
 ...
 if(form)form.onsubmit=async function(e){e.preventDefault();var q=String(input.value||'').trim();if(!q)return;body.innerHTML+='<div class="znext-ai-msg znext-ai-user">'+esc(q)+'</div>';input.value='';var msg=document.createElement('div');msg.className='znext-ai-msg';msg.innerHTML='⏳ '+(ar()?'أبحث في قاعدة معرفة الموقع والمحتوى المنشور...':'Searching the website knowledge base and published content...');body.appendChild(msg);await build();msg.innerHTML=answer(q);body.scrollTop=body.scrollHeight;};
}
function boot(){renderAssistant();build();setTimeout(renderAssistant,500);setTimeout(build,2500)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
setInterval(function(){renderAssistant();build()},120000);
})();
